...
run: insert-arg
	...
	@$(MAKE) -C "$(NPC_HOME)" sim-iverilog IMG="$(abspath $(IMAGE).bin)" MAX_CYCLES="$(NPC_MAX_CYCLES)" TIMEOUT_OK="$(NPC_TIMEOUT_OK)" WAVE="$(WAVE)"
...
run: insert-arg
	...
	@$(MAKE) -C "$(NPC_HOME)" iverilog-netlist-run IMG="$(abspath $(IMAGE).bin)" MAX_CYCLES="$(NPC_MAX_CYCLES)" TIMEOUT_OK="$(NPC_TIMEOUT_OK)" WAVE="$(WAVE)"
else
run: insert-arg
